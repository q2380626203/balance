...
- **平衡控制算法重大优化**: 重构YAW角变化检测和电机重启机制
  - **智能变化检测**: 改为基于时间窗口(500ms)和变化幅度(1°)的综合判断
    - 位置：`main/main.cpp:125-141`
    - 逻辑：只有在500ms内YAW角变化≤1°时才触发电机重启，避免误判
  - **电机重启冷却机制**: 加入1.5秒冷却时间，防止频繁重启
    - 位置：`main/main.cpp:185-186`
    - 效果：提升系统稳定性，减少不必要的电机操作
  - **重启后高频控制**: 重启后500ms内以10ms频率高频发送速度指令
    - 位置：`main/main.cpp:251-266`
    - 作用：确保电机重启后快速响应，提升控制精度
  - **定期错误清理**: 新增每3秒自动清理电机错误的机制
    - 位置：`main/main.cpp:268-273`
    - 功能：主动维护电机状态，提升长期运行稳定性

### 2025-07-22
- **平衡控制优化**: 将电机调整方向取反，改善平衡响应效果
  - 修改位置：`main/main.cpp:198, 229`
  - 控制逻辑：YAW误差正值时电机反向转动，误差负值时正向转动
